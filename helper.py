...
def execute_query(ns, query):
    ...
    logger.info(f"{ get_query_path() }: Query took {elapsed_time_ms:.2f} seconds. Documents found: {len(results)}. query: {json.dumps(query, default=JSONEncoder)}")
...
def execute_aggregation(ns, pipeline):
    ...
    logger.info(f"{ get_query_path() }: Query took {elapsed_time_ms:.2f} seconds. Documents found: {len(results)}. query: {json.dumps(pipeline, default=JSONEncoder)}")
